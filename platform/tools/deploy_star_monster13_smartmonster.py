...
DEFAULT_DONOR_CLIENT = Path(r"E:\星辰剑歌")
DEFAULT_SERVER_ROOT = Path(r"D:\MirServer")
DEFAULT_TARGET_DATA = Path(r"E:\11周年\data")
DEFAULT_CANDIDATE_ROOT = (
    PLATFORM_ROOT / "怪物库" / "simulation" / "20260823_star13_smartmonster" / "candidate"
)
DEFAULT_BACKUP_ROOT = PLATFORM_ROOT / "怪物库" / "backups" / "star13-smartmonster"
DEFAULT_REPORT_ROOT = PLATFORM_ROOT / "怪物库" / "build" / "star13-smartmonster"

SOURCE_MONSTER = "祖玛力士"
TARGET_MONSTER = "星辰怪13"
SOURCE_EFFECT_INDEX = 78
SOURCE_RESOURCE_ENTRY = "Mon7.wzl"
TARGET_RESOURCE_STEM = "XY_StarMon13"
TARGET_RESOURCE_ENTRY = f"{TARGET_RESOURCE_STEM}.wzl"
EXPECTED_SOURCE_HASHES = {
    "ini": "DCEA665F9095D82C4FFE4F44571E5FCCA26CEB0A403871FCF705855CE0E24B1A",
...
EXPECTED_TARGET_MODEL = {"Race": 156, "RaceImg": 156, "Appr": 1123}
RESOURCE_KEYS = {
    "HPFile",
    "ActionFile",
    "EffectFile",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="星辰怪13 SmartMonster完整动作链：默认构建隔离候选并只读预检"
    )
...
def build_candidate_ini(source: Path, target_index: int) -> tuple[bytes, dict[str, object]]:
    ...
    if resource_counts.get("ActionFile", 0) == 0 or unexpected_refs:
        raise RuntimeError(f"供体INI存在未纳入的资源依赖：{unexpected_refs}")
    output_lines: list[str] = []
    replaced = 0
    for line in text.splitlines():
        key, separator, value = line.strip().partition("=")
        if separator and key in RESOURCE_KEYS and value == str(SOURCE_EFFECT_INDEX):
            output_lines.append(f"{key}={target_index}")
            replaced += 1
        else:
            output_lines.append(line)
    expected_replacements = sum(resource_counts.values())
    if replaced != expected_replacements:
        raise RuntimeError(f"资源引用重写数量异常：{replaced}/{expected_replacements}")
    ...
    rewritten = sorted({
        ...
        for key, value in candidate_parser.items(section)
        if key in RESOURCE_KEYS and int(value) >= 0
    })
    if rewritten != [target_index]:
        raise RuntimeError(f"候选INI仍有错误资源引用：{rewritten}")
    return candidate, {
        "source_action_file": SOURCE_EFFECT_INDEX,
        "target_action_file": target_index,
        "rewritten_occurrences": replaced,
        "rewritten_resource_keys": resource_counts,
        "encoding": encoding,
...
def build_candidate(args: argparse.Namespace, effect: dict[str, object]) -> dict[str, object]:
    ...
    manifest = {
        "schema": "xydp.star13-smartmonster-candidate.v1",
...
def preflight(args: argparse.Namespace) -> dict[str, object]:
    ...
    result = {
        "schema": "xydp.star13-smartmonster-preflight.v1",
        "created_at": dt.datetime.now().astimezone().isoformat(timespec="seconds"),
        "status": status,
        "source": sources,
        "candidate": candidate,
...
def apply(args: argparse.Namespace, plan: dict[str, object]) -> dict[str, object]:
    ...
    try:
        ...
        receipt = {
            **plan,
            "schema": "xydp.star13-smartmonster-deploy.v1",
...
def rollback(args: argparse.Namespace, transaction_id: str) -> dict[str, object]:
    ...
    if not receipt["backups"]["absence"]["smart_dir_existed"]:
        try:
            ...
            pass
    result = {
        "schema": "xydp.star13-smartmonster-rollback.v1",
        "status": "rolled-back",
        "transaction_id": transaction_id,
        "completed_at": dt.datetime.now().astimezone().isoformat(timespec="seconds"),
